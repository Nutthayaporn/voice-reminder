-- Add clock-style snooze and "remind until done" policy to existing installs.

alter table public.items
  add column if not exists alert_mode text not null default 'notification',
  add column if not exists remind_until_done boolean not null default false,
  add column if not exists snooze_minutes integer not null default 10,
  add column if not exists max_attempts integer not null default 5;

alter table public.items drop constraint if exists items_alert_mode_check;
alter table public.items
  add constraint items_alert_mode_check check (alert_mode in ('notification', 'alarm'));

alter table public.items drop constraint if exists items_snooze_minutes_check;
alter table public.items
  add constraint items_snooze_minutes_check check (snooze_minutes in (5, 10, 30));

alter table public.items drop constraint if exists items_max_attempts_check;
alter table public.items
  add constraint items_max_attempts_check check (max_attempts between 1 and 20);
