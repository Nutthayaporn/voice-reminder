...
create table public.gmail_connections (
  ...
  tokens text not null, -- AES-GCM encrypted with GMAIL_TOKEN_KEY
...
grant all on public.gmail_connections, public.gmail_oauth_states to service_role;
...
create function public.complete_gmail_oauth(p_state_hash text, p_tokens text)
returns boolean language plpgsql security definer set search_path = '' as $$
declare pending public.gmail_oauth_states;
begin
  delete from public.gmail_oauth_states
    where state_hash = p_state_hash and expires_at > now() returning * into pending;
  if not found then return false; end if;
  insert into public.gmail_connections(user_id, provider, tokens, updated_at)
...
$$;
revoke all on function public.complete_gmail_oauth(text, text) from public, anon, authenticated;
grant execute on function public.complete_gmail_oauth(text, text) to service_role;
